...
def pre_save_rates(sender, instance, **kwargs):
    if instance.description == None:
        instance.description = ''

@receiver(post_save, sender=PageTitle)
def post_save_pt_rates(sender, instance, created, **kwargs):
    if created:
        Seo(page_title=instance).save()


@receiver(pre_save, sender=Seo)
def pre_save_seo_rates(sender, instance, **kwargs):
